...
def generate_cypher(client, question, history):
    system = (
        ...
        "-[:PICK_UP_AT]->DistributionRegion-[:SUPPLIES_TO]->Store.\n"
        "- A Warehouse's own distribution region is found via "
        "Warehouse-[:LOCATED_IN]->Location-[:PICK_UP_AT]->DistributionRegion "
        "— use this directly for questions like 'how many warehouses are in "
        "region X' or 'what region is warehouse Y in'. Do NOT route through a "
        "Store's Location to answer a question about warehouses.\n"
        "- A Store's own distribution region is found via "
        "DistributionRegion-[:SUPPLIES_TO]->Store — use this directly for "
        "questions about which region supplies/serves a store. Do NOT match "
        "on Location to find a store's region; a warehouse and a store can "
        "share the same Location (same city) without being in a "
        "supplier relationship, and most stores share no city with any "
        "warehouse at all.\n"
